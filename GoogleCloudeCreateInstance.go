...
func createWithLocalSSD(projectID, zone, instanceName,machineType, sourceImage, networkName, accountadc_config string) error{
...
func createFirewallRule(projectID, firewallRuleName, networkName string)  {
	// projectID := "your_project_id"
...
func main()  {
	...
	flag.StringVar(&zone, "z", "asia-east2-b", "区域,默认：asia-east2-b（香港b区）")
	flag.StringVar(&instanceName, "i", "instance", "实例名称，默认：instances+数字序列")
	flag.StringVar(&machineType, "t", "e2-standard-4", "实例类型,默认：e2-standard-4(4c16g)")
	...
	flag.IntVar(&instanceNum, "N", 4, "申请实例数量,默认：4")
	...
	if Firewald ==1{
		createFirewallRule(projectID,"default-allow-all",NetworkName)
	}
	for i := 1; i<= instanceNum; i++ {
		if projectID != "" {
			...
			fmt.Printf("上述操作完成以后，使用命令：gcloud auth application-default login ，弹出浏览器登陆googlecloud帐号，生成配置文件，\n"+
				"如有多帐号，请使用：gcloud auth application-default revoke 退出登陆，再登陆新帐号。\n")
...
////test main
//func  main() {
//	projectID := "fifth-boulder-375314"
//	instanceName := "instance-1"
//	zone := "asia-east1-b"
//	accountadc_credentials_config := "/home/rowe.config/gcloud/application_default_credentials.json"
//	SetCommonInstanceMetadata(projectID, instanceName, zone, accountadc_credentials_config)
//}
